            ...
            with torch.cuda.amp.autocast(enabled=(self.device == 'cuda')):
                for images, masks, names in tqdm(self.val_loader, desc='Val'):
                    ...
                    for name, image, mask, pred in zip(names, images_cpu, masks_cpu, preds_cpu):
                        ...
                        #print(f"Sample: {name} | Dice CUP | DISC : {dice_c.item():.4f}|{dice_d.item():.4f} | IoU CUP | DISC : {iou_c.item():.4f}|{iou_d.item():.4f}")
                        #dice_c_item = dice_c.item()
                        #dice_d_item = dice_d.item()
                        #iou_c_item = iou_c.item()
                        #iou_d_item = iou_d.item()
